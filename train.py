...
from model import XSpkEmoTransLoss
...
def train(rank, args, configs, batch_size, num_gpus):
    ...
    if rank == 0:
        print("Number of XSpkEmoTrans Parameters: {}\n".format(get_param_num(model)))
        # Init logger
            ...
            for batch in batchs:
                ...
                if rank == 0:
                    if step % log_step == 0:
                        ...
                        message2 = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Emotion Classifier Loss: {:.4f}, Duration Loss: {:.4f}".format(
                            *losses
                        )
                    ...
                    if step % val_step == 0:
                        model.eval()
                        message = evaluate(
                            device, model, step, configs, val_logger, vocoder, len(losses))
